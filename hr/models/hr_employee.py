...
class HrEmployeePrivate(models.Model):
    ...
    def _compute_ranking(self):
        ...
        previous_performance_score = None
        previous_rank = None
        for i, record in enumerate(sorted_records):
            if record.performance_score != previous_performance_score:
                # Nếu performance_score của bản ghi hiện tại không bằng với performance_score của bản ghi trước đó
                # thì tăng hạng lên 1
                rank = i + 1
            else:
                # Nếu performance_score của bản ghi hiện tại bằng với performance_score của bản ghi trước đó
                # thì sử dụng cùng một hạng
                rank = previous_rank
            record.ranking_sort = rank
            # Lưu lại performance_score và hạng cho việc kiểm tra với bản ghi tiếp theo
            previous_performance_score = record.performance_score
            previous_rank = rank
